...
sec_session_start();
	...
	$sql = "SELECT AVG(time) FROM $tableName WHERE difficulty = $difficultyInput AND username = ANY(SELECT studentUsername from students where teacherId = (SELECT teacherID from teachers where username = '$username'))";
	...
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
		$arrayResults[] = array($row['AVG(time)']);
	}
