...
//Chnage SQL to only import a batch of words
	...
	if($gameName == "Zebra") {
		$sql = "SELECT failed1,failed2,failed3 FROM $tableName WHERE difficulty = $difficultyInput AND username = '$username'";
		...
		$sql = "SELECT failed FROM $tableName WHERE difficulty = $difficultyInput AND username = '$username'";
		$query = mysql_query($sql,$conn);
		$arrayResults = array();
		while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
