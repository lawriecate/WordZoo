...
$('#graph').on('change', updateChart)
 ...
 function updateChart(){
	...
	if(sgraphName == "Scores"){
		//Draw Line Graph
		isLine = true;
		importGraphData(sgameName,sdifficultyName);
		ochart = new Chart(ctx).Line(data);
	} else if (sgraphName == "Most Failed") {
		//Draw Bar Chart
		isLine = false;
		var importedData = importMostFailed(sgameName, sdifficultyName);
	...
	$('#averageScore').text("Average score: " + importStatistics(sgameName, sdifficultyName, 'php/importAverageScore.php'));
	$('#highestScore').text("Highest score: " + importStatistics(sgameName, sdifficultyName, 'php/importMaxScore.php'));
	$('#highestScorer').text("Highest scorer: " + importStatistics(sgameName, sdifficultyName, 'php/importHighestScorer.php'));
	if(sgameName != "Zebra") {
		$('#averageResponseTime').text("Average response time: " + importStatistics(sgameName, sdifficultyName, 'php/importAverageResponseTime.php') + " seconds");
	} else {
		$('#averageResponseTime').text("");
	}
	$('#averagePlayTime').text("Average play time: " + importStatistics(sgameName, sdifficultyName, 'php/importAverageSessionTime.php') + " seconds");
	
	getChosenWords("Elephant", "Hard", "");
 }
 
function importGraphData(sgameName, sdifficultyName) {
	$.ajax({
      method: 'get',
      url: 'php/importScoreOverTime.php',
      dataType: 'json',
      async: false,
	  data: {
		'gameName': sgameName,
      ...
      success: function(data) {  
		...
		for(var i = 0; i<data.length; i++) {
			score[i] = Math.floor(data[i][0]);
			dateTime[i] = data[i][1];
            dateTime[i] = dateTime[i].split(" ")[0];
		}
      }
    });  	
}
